...
function repositionFloatingBubbles() {
  ...
  // Keep the MiniCPM chat bubble glued to the pet when it moves.
  try { if (_minicpmChat && _minicpmChat.isOpen()) _minicpmChat.reposition(); } catch {}
  return result;
}
...
const _minicpmChat = require("./minicpm-chat")({
  getPetWindowBounds,
  // Hit-rect = the actual visible character on screen (smaller than the
  // pet window which has transparent margins). Anchoring the bubble to
  // this rect keeps it visually adjacent to the sprite, not floating off
  // a window edge.
  getPetHitRect: () => {
...
//   • overload      — a session has been in `working` continuously > N ms
//   • error-streak  — a session has fired ≥ K StopFailure / PostToolUseFailure
//                     events without a clean Stop in between
// The derived state is itself POSTed back to /state with a sentinel flag
// (`derived: true`) so the same handler doesn't re-process it and recurse.
const _derived = (() => {
  const { postStateToRunningServer } = require("../hooks/server-config");
  const OVERLOAD_AFTER_MS = Number(process.env.MINICPM_OVERLOAD_MS || 5 * 60 * 1000);
  const FAILURE_STREAK_THRESHOLD = Number(process.env.MINICPM_FAILURE_STREAK || 3);

  const workingSince = new Map();   // session_id -> timestamp ms
  const overloadFired = new Set();  // session_id where overload already pushed
  const failureCount = new Map();   // session_id -> consecutive failures

  function pushDerived(body) {
    body.derived = true;
    postStateToRunningServer(JSON.stringify(body), { timeoutMs: 200 }, () => {});
  }

  return function process(data) {
    if (!data || typeof data !== "object" || data.derived === true) return;

    const sid = String(data.session_id || "default");
    const event = String(data.event || "");
    const state = String(data.state || "");

    // ── Long-task / overload detection ──
    if (state === "working") {
      if (!workingSince.has(sid)) workingSince.set(sid, Date.now());
      const dur = Date.now() - workingSince.get(sid);
      if (dur > OVERLOAD_AFTER_MS && !overloadFired.has(sid)) {
        overloadFired.add(sid);
        pushDerived({
          state: "overload",
          event: "OverloadDetected",
          agent_id: data.agent_id || "unknown",
          session_id: sid + ":overload",
          cwd: data.cwd,
          source_session: sid,
          duration_ms: dur,
        });
        console.log(`[derived] overload: session=${sid.slice(0,8)} working ${(dur/1000)|0}s`);
      }
    } else if (state === "idle" || state === "attention" || state === "sleeping" || event === "Stop" || event === "StopFailure") {
      // Reset the overload tracker as soon as the session leaves working.
      workingSince.delete(sid);
      overloadFired.delete(sid);
    }

    // ── Failure streak detection ──
    if (event === "StopFailure" || event === "PostToolUseFailure") {
      const n = (failureCount.get(sid) || 0) + 1;
      failureCount.set(sid, n);
      if (n >= FAILURE_STREAK_THRESHOLD) {
        pushDerived({
          state: "error",
          event: "FailureStreak",
          agent_id: data.agent_id || "unknown",
          session_id: sid + ":streak",
          cwd: data.cwd,
          source_session: sid,
          consecutive_failures: n,
        });
        console.log(`[derived] failure-streak: session=${sid.slice(0,8)} count=${n}`);
        // Don't reset — keep emitting on every additional failure so the user
        // really notices. The counter resets on the next clean Stop.
      }
    } else if (event === "Stop" || event === "SessionStart" || event === "PostToolUse") {
      // Clean event seen → reset the streak.
      failureCount.delete(sid);
    }
  };
})();
function processDerivedStates(data) { return _derived(data); }
...
const _serverCtx = {
  ...
  permLog,
  // Side-channel: forward each /state POST body to the MiniCPM narrator so
  // it can decide whether to fire a one-line reaction from the local model.
  // Also feed the derived-state monitor (overload / failure-streak).
  onStateEvent: (data) => {
    try {
    ...
    try { processDerivedStates(data); } catch (err) { console.warn("derived-states:", err && err.message); }
  },
};
...
function createWindow() {
  ...
  ipcMain.on("open-minicpm-chat", () => { try { openMinicpmChat(); } catch (err) { console.error("openMinicpmChat:", err); } });
  ...
  registerPetInteractionIpc({
    ...
    setDragLocked: (value) => {
      ...
      // Notify the MiniCPM chat bubble so it can fade out during drag and
      // recompute its anchor side after the drop — otherwise the bubble
      // sticks to its original side and visually overlaps the pet at
      // screen edges.
      try {
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    ...
    // ── MiniCPM chat: opinionated defaults ─────────────────────────────────
    // The Session HUD is meant for coding-agent integrations (Claude Code,
    // Codex, etc.) and is just confusing empty chrome on a chat-only pet.
    // Turn it off so hovering the pet doesn't show useless info.
    try {
      if (_settingsController.get("sessionHudEnabled") !== false) {
        _settingsController.applyUpdate("sessionHudEnabled", false);
      }
    } catch (err) {
      console.warn("Clawd: could not disable Session HUD:", err && err.message);
    }

    // ── MiniCPM chat: global hotkeys ───────────────────────────────────────
    //   ⌘⇧M / ⌃⇧M  : open / close the chat bubble
    //   ⌘⇧T / ⌃⇧T  : toggle "show thinking" inside the bubble
    try {
      ...
      globalShortcut.register("CommandOrControl+Shift+T", () => {
        try {
          // If the bubble window doesn't exist yet, opening + toggling needs
          // a render-side listener. Easiest is to open it briefly so the
          // listener attaches, then forward the toggle.
          if (!_minicpmChat.isOpen()) _minicpmChat.toggle();
          _minicpmChat.toggleThinking && _minicpmChat.toggleThinking();
        } catch {}
      });
    } catch (err) {
    ...
    // ── MiniCPM chat: warm up the sidecar at app launch ───────────────────
  ...
  app.on("before-quit", () => {
    ...
    try { _minicpmChat.shutdown(); } catch {}
